...
const LineChart: React.FC<LineChartProps> = ({ data }) => {
  ...
  switch (data.scale) {
    ...
    case "Week":
      ...
      timestampThreshold = new Date(
        nowCopy.setDate(nowCopy.getDate() - 7)
      ).getTime();
      const preLabels2 = [
        "Monday",
        "Tuesday",
    ...
    case "Month":
      ...
      const preLabels3 = Array.from(
        Array(new Date(now.getFullYear(), now.getMonth(), 0).getDate()).keys()
      );
  ...
  data.data.garbage.forEach((garbage) => {
    const existingGarbage = dataGroupedByCategory.garbage.find(
      (item) => item.category === garbage.category
    );
    if (existingGarbage) {
      existingGarbage.values = existingGarbage.values.concat(
        garbage.values.filter(
          (item) => new Date(item.date).getTime() >= timestampThreshold
        )
      );
      return;
    }
          ...
          datasets: dataGroupedByCategory.garbage.map((item) => {
            return {
              ...
              data: item.values.reduce((total, current) => {
                let index =
                  +new Date(current.date)[orientationDateMethodName]() -
                  now[orientationDateMethodName]() -
                  1;
                if (index < 0) {
                  index += max;
                }
